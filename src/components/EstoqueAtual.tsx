...
export default function EstoqueAtual({ inventory, onDelete, onEdit, onEditName }: EstoqueAtualProps) {
  ...
  const getSizeColor = (): { bg: string; text: string; border: string } => {
  ...
  return (
    <>
      ...
      <AlertDialog open={deleteDialog.isOpen} onOpenChange={(open: boolean) => setDeleteDialog({ isOpen: open, item: deleteDialog.item })}>
        <AlertDialogContent>
          <AlertDialogHeader>
